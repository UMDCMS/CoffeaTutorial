...
For starting the coffea session after fresh logging into a machine. If you want
use jupyter, remember to connect with the SSH tunneling settings:

.. code-block:: sh

  ssh -L localhost:8XXX:localhost:8XXX <user>@<machine>

Or for GUI based settings: find Connection/Tunneling settings for your ssh
session. Set the **source port** to ``8XXX`` and the **destination** to
``127.0.0.1:8XXX``. Swap out ``XXX`` to you favorite 3 digit number. For each new
session.
...
The tutorial follows using `jupyter notebook <https://jupyter.org/>`_ to
demonstrate a typical coffea work flow. If you are unfamiliar with using
notebooks, see the `notebook <notebook.html>`_ page. If you are more familiar
with script based work flows, you can still join all the relevant code cells in
the notebook together into a single script, and it should be able to executed
...
If you already have a working coffea environment, feel free to ignore this
section. The following section is aimed for people looking do design or modify
...
for the sake of people who want to follow along notebook by notebook or want to
setup an environment to help with contributing to tutorial. Notice that this
would be a separate install from the instructions above.

To install

-- code-block:: sh
  git clone git@github.com:UMDCMS/CoffeaTutorial.git <TUTORIALDIR>
  cd <TUTORIALDIR>
  chmod +x setup.sh # or setup_LCG.sh on a LCG machine
  ./setup_LCG.sh    # or ./setup_LCG.sh
